                    ...
                    if (response.success)
                    {
                        ...
                        using (var ds = db.ExecuteWithResults(sql))
                        {
                            ...
                            if (response.success)
                            {
                                ...
                                var sql2 = "select roleid id,rolename text from n_role where roleid in(select roleid from n_roleuser where siteid="+user.siteid+" and userid=" + user.userid + ") order by seqno;select orgid id,orgname text,parentid from n_org where siteid=" + user.siteid + " and orgid in(select orgid from n_orguser where userid=" + user.userid + ") order by seqno";
