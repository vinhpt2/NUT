...
export class NWin {
	...
	buildWindow(div, conf, tabLevel, callback) {
		...
		for (var i = 0; i < conf.tabs.length; i++) {
			...
			if (tabconf.tablevel == tabLevel) {
				var divTab = div.z(["div", { id: "tab_" + tabconf.tabid, style: "height:" + (tabconf.maxLevel ? "45%" : "95%")}]);
	...
	buildContent(div, conf, callback) {
		...
		for (var i = 0; i < conf.fields.length; i++) {
			...
			if (!fldconf.hideingrid) {
				var column = { field: fldconf.columnname, text: alias, size: (fldconf.displaylength||100)+"px", sortable: true, frozen: fldconf.isfrozen, resizable: true, searchable: !fldconf.hideinfind, options: { conf: fldconf } };
		...
		for (var i = 0; i < fields.length; i++) {
			if(!fields[i].options.conf.hideinfind){
				var fld=NUT.clone(fields[i]);
				fld.required = false;
				fld.html.column = (NUT.isMobile ? 0 : (fld.colspan ? fld.colspan - 1 : i++ % (conf.layoutcols || NUT.isGIS?2:3)));
				searches.push(fld);
			}
		}
		...
		if (!viewonly && !access.noinsert) items.push({ type: (isNewGeo ? "menu" : "button"), id: (isNewGeo ? "new" : "NEW"), text: '📄', tooltip: "_New", items: [{ id: "NEW", text: "_New" }, { id: "NEW_G", text: "_NewG" }] });
		...
		items.push({ type: 'break' });
		if (!viewonly&&!access.noupdate && conf.relatetableid) items.push({ type: 'button', id: "LINK", text: '🔗', tooltip: "_Link Data" });
	...
	static fieldsFromConfig(conf){
		...
		for (var i = 0; i < conf.fields.length; i++){
			...
			if(!fldconf.hideinform) {
				var alias = NUT.translate(fldconf.translate) || fldconf.fieldname;
				var field = { field: fldconf.columnname, type: fldconf.fieldtype, required: fldconf.isrequire, disabled: fldconf.isreadonly, label: alias, html: { label: alias, column: (NUT.isMobile ? 0 : (fldconf.colspan ? fldconf.colspan - 1 : index++ % (conf.layoutcols || NUT.isGIS ? 2 : 3))), attr: "tabindex=0" }, options: { conf: fldconf } };
				var labspan = conf.labelspan || (NUT.isMobile || NUT.isGIS ? -1 : null);
				if (labspan) {
				...
				onOpen(evt) {
					evt.onComplete = function () {
						...
						var frmNew = (w2ui[id] || new w2form({
							name: id,
							fields: NWin.fieldsFromConfig(conf),
							onChange: this.field_onChange,
							record: conf.default,
							actions: {
								"_Close": function () {
							...
							for (var i = 0; i < changes.length; i++) {
								...
								if (!NUT.isObjectEmpty(change)) {
									...
									if (conf.beforechange) {
										...
										} else NUT.ds.update({ url: conf.table.urledit, where: [columnkey, "=", recid], data: data }, callback);
									}
									hasChanged = true;
								}
							}
	...
	grid_onRequest(evt) {
		...
		var reqData = tabconf.table.tabletype=="arcgis" ? { resultRecordCount: postData.limit, resultOffset: postData.offset, f: "geojson", outFields: "*", returnGeometry:false, token: NUT.AGMap.token } : { limit: postData.limit, offset: postData.offset };
	...
	grid_onLoad(evt) {
		...
		var total = evt.detail.data.total||0;
